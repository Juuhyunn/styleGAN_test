...
class Train:
    ...
    def process(self):
        ...
        g_ema.eval()
        self.accumulate(g_ema, generator, 0)
        ...
        dataset = MultiResolutionDataset('C:\ProgramData\Anaconda3\envs\pytorch18\Lib\site-packages\lmdb', transform, args.size)
        loader = data.DataLoader(
            dataset,
            batch_size=args.batch,
            sampler=self.data_sampler(dataset, shuffle=True, distributed=args.distributed),
            drop_last=True,
        )
        ...
        self.train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device)
    ...
    def set_grad_none(self, model, targets):
        for n, p in model.named_parameters():
            if n in targets:
                p.grad = None

    def train(self, args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
        ...
        for idx in pbar:
            ...
            if args.finetune_loc <= 0:
                ...
                for loc in range(args.finetune_loc):
                    self.requires_grad(generator, False, target_layer=f'convs.{generator.num_layers - 2 - 2 * loc}')
                    self.requires_grad(generator, False, target_layer=f'convs.{generator.num_layers - 3 - 2 * loc}')
                    self.requires_grad(generator, False, target_layer=f'to_rgbs.{generator.log_size - 3 - loc}')
            self.requires_grad(discriminator, True)
            noise = self.mixing_noise(args.batch, args.latent, args.mixing, device)
            ...
            if args.finetune_loc <= 0:
                ...
                for loc in range(args.finetune_loc):
                    self.requires_grad(generator, True, target_layer=f'convs.{generator.num_layers - 2 - 2 * loc}')
                    self.requires_grad(generator, True, target_layer=f'convs.{generator.num_layers - 3 - 2 * loc}')
                    self.requires_grad(generator, True, target_layer=f'to_rgbs.{generator.log_size - 3 - loc}')
            ...
            if args.finetune_loc < 0 and g_regularize:
                ...
                noise = self.mixing_noise(path_batch_size, args.latent, args.mixing, device)
